...
LAMMPS MD trajectory files (.lammpstrj) using the hbond_topology package.

Note on LAMMPS dump files:
  - The dump file must contain element symbols (not just numeric types).
  - Use: dump_modify 1 element O H Si ...
  - Box dimensions are read directly from the dump file header.
  - No separate topology (.data) or cell file is needed.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='LAMMPS MD H-bond Topology Analysis (Extended)',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    
    # Trajectory parameters
    parser.add_argument('--trajectory', '-t', type=str, default='trajectory.lammpstrj',
                        help='Path to LAMMPS dump trajectory file (.lammpstrj)')
    parser.add_argument('--timestep', type=float, default=1.0,
                        help='MD timestep in femtoseconds (depends on LAMMPS units style)')
    parser.add_argument('--sample-interval', type=int, default=1,
                        help='Analyze every N frames')
    parser.add_argument('--data-file', '-d', type=str, default='model.lmpdat',
                        help='Path to LAMMPS data file (.data/.lmpdat) for '
                             'auto-extracting atom type → element mapping '
                             'from the Masses section')
    parser.add_argument('--type-map', type=str, default=None,
                        help='Manual atom type → element mapping, overrides '
                             '--data-file. Format: "1:O 2:H 3:Si 4:C"')
...
def main():
    """Run comprehensive H-bond topology analysis on LAMMPS MD trajectory."""
...
def _main_body(args, traj_file: Path, output_dir: Path, log_path: Path):
    ...
    print("=" * 70)
    print("LAMMPS MD Hydrogen Bond Topology Analysis")
    print("=" * 70)
    ...
    # Parse trajectory
    # LAMMPS dump files (.lammpstrj) contain box info in the header,
    # so no separate cell file is needed (unlike CP2K XYZ).
    # However, numeric atom types must be mapped to element symbols
