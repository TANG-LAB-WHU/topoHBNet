# LAMMPS AIMD: Water/Substrate Interface (No Field)
...
> [!NOTE]
> These four analysis workflows are completely independent. They can be executed in any order (or even concurrently in parallel), as each script only processes the raw output files from the LAMMPS MD simulation (`trajectory.lammpstrj`, `model.lmpdat`, and `log.lammps`) and does not depend on the outputs of the other scripts.
...
- **Usage**:
  - **To generate topological analysis results without machine learning (`topoHBNet-no_run-ml/`):**
    ```bash
    python topoHBNet_main_analysis.py --trajectory trajectory.lammpstrj --data-file model.lmpdat --output-dir topoHBNet-no_run-ml
    ```
  - **To generate topological analysis results with machine learning enabled (`topoHBNet-run-ml/`):**
    ```bash
    python topoHBNet_main_analysis.py --trajectory trajectory.lammpstrj --data-file model.lmpdat --run-ml --output-dir topoHBNet-run-ml
    ```
...
- **Usage**:
  - **To generate species analysis results (`trajectory_species_results/`):**
    ```bash
    python trajectory_species_analysis.py --trajectory trajectory.lammpstrj --data-file model.lmpdat --output-dir trajectory_species_results
    ```
...
A utility script to verify simulation stability by parsing the energetics output from LAMMPS.
...
- **Usage**:
  - **To generate energetics visualization results (`visualization_aimd_energetics/`):**
    ```bash
    python visualizing_aimd_energetics.py --input log.lammps --target-temp 298.0 --output visualization_aimd_energetics
    ```
...
- **Usage**:
  - **To generate interfacial water analysis results (`interfacial_analysis_results/`):**
    ```bash
    python interfacial_water_analysis.py --trajectory trajectory.lammpstrj --data-file model.lmpdat --mode dynamic --output-dir interfacial_analysis_results
    ```
...
- `trajectory.lammpstrj`: The molecular positions for each frame (includes Box bounds).
- `model.lmpdat`: LAMMPS data file containing atom masses for element identification (alternatively use `--type-map`).
- `log.lammps`: LAMMPS energy and temperature thermo log.
