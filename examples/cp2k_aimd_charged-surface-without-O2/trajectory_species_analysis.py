...
Usage:
    python trajectory_species_analysis.py                        # uses trajectory.xyz + trajectory.cell
    python trajectory_species_analysis.py --cell 22.0 19.08 46.35
    python trajectory_species_analysis.py --xyz system-pos-1.xyz --cell 10 10 10 \
           --roh 1.3 --stride 10 --output-dir results/
"""
...
def plot_species_pie(
    ...
    # Export raw pie chart distribution data to CSV
    csv_path = Path(output_path).with_suffix(".csv")
    total_val = sum(values)
    dist_df = pd.DataFrame({
        "Species": labels,
        "Average_Count": values,
        "Percentage": [v / total_val * 100 if total_val > 0 else 0.0 for v in values]
    })
    dist_df = dist_df.sort_values(by="Average_Count", ascending=False)
    dist_df.to_csv(csv_path, index=False)
    print(f"  Distribution CSV saved: {csv_path}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  %(prog)s --xyz traj.xyz --cell 10 10 10         # custom xyz (plots generated by default)\n"
